...
List<Debtor> debtors = new List<Debtor> {
            ...
            new Debtor("Pamela H. Beauchamp", DateTime.Parse("November 20, 1959"), "123-456-7890", "PamelaHBeauchamp@jourrapide.com", "3239 Tori Lane Salt Lake City, UT 84104", 6588),
...
var db1 = debtors.Where(db => db.Email.EndsWith("rhyta.com") || db.Email.EndsWith("dayrep.com")).ToList();

//db1.ForEach(db => Console.WriteLine(db));
//=========================================
...
var db2 = debtors.Where(db => d.Year - db.BirthDay.Year >= 26 && d.Year - db.BirthDay.Year <= 36).ToList();

//db2.ForEach(db => Console.WriteLine(db.BirthDay));
//========================================

//3) Borcu 5000-den cox olmayan borclularic cixartmag

var db3 = debtors.Where(db => db.Debt <= 5000).ToList();

//db3.ForEach(db => Console.WriteLine(db.Debt));
//========================================

//4) Butov adi 18 simvoldan cox olan ve telefon nomresinde 2 ve ya 2-den cox 7 reqemi olan borclulari cixartmaq

var db4 = debtors.Where(db => db.FullName.Length >= 18 && (db.Phone.Count(n => n == '7') >= 2)).ToList();

//db4.ForEach(db => Console.WriteLine($"{db.FullName} {db.Phone}"));
//========================================


//5) Qishda anadan olan borclulari cixardmaq

var db5 = debtors.Where(db => db.BirthDay.Month == 12 || db.BirthDay.Month == 1 || db.BirthDay.Month == 2).ToList();

//db5.ForEach(db => Console.WriteLine($"{db.FullName} {db.BirthDay}"));
//========================================
...
//double debt = debts /  debtors.Count;

//var db6 = debtors.Where(db => db.Debt > debt).OrderByDescending(db => db.FullName).OrderByDescending(db => db.Debt).ToList();

//db6.ForEach(db => Console.WriteLine($"{db.FullName} {db.Debt}"));
//=========================================

//7) Telefon nomresinde 8 olmayan borclularin yalniz familyasin, yashin ve umumi borcun meblegin cixarmaq

//var db7 = debtors.Where(db => !db.Phone.Contains('8')).ToList();

//db7.ForEach(db => Console.WriteLine(db.Phone));
//========================================
...
//var db8 = debtors.Where(db =>
//{

//    for (int i = 0; i < db.FullName.Length; i++)
//    {
//        int count = 0;
//        for (int j = 0; j < db.FullName.Length; j++)
//        {
//            if (db.FullName[i] == db.FullName[j])
//            {
//                count++;
//            }
//        }
...
//    }
//    return false;
//}).OrderByDescending(db => db.FullName).ToList();

//db8.ForEach(db => Console.WriteLine(db.FullName));
//=============================================
...
//    {
//        if (debtors[i].BirthDay.Year == debtors[j].BirthDay.Year)
//        {
//            count++;
//        }
//    }
...
//    {
//        count2= count;
//        year = debtors[i].BirthDay.Year;
//    }
//}


//var db9 = debtors.Where(db => db.BirthDay.Year == year).ToList();

//db9.ForEach(db => Console.WriteLine($"{db.FullName} {db.BirthDay.Year}"));
//================================================


//10)Borcu en boyuk olan 5 borclunun siyahisini cixartmaq

//var db10 = debtors.OrderByDescending(dt => dt.Debt).ToList();
//int count = 0;
//db10.ForEach(db =>
//{
//    if (count < 5)
//    {
//        Console.WriteLine($"{db.FullName} {db.Debt}");
//        count++;
//    }
...
//int debt = 0;
//var db11 = debtors.Where(db => (debt+= db.Debt)> 0).ToList();
...
//var db12 = debtors.Where(db => db.BirthDay.Year <= 1945).ToList();

//db11.ForEach(db => Console.WriteLine($"{db.FullName} {db.BirthDay.Year} {db.Debt}"));


//13)Nomresinde tekrar reqemler olmayan borclularin ve onlarin borcunun meblegin cixartmaq


//var db13 = debtors.Where(db =>
//{
//   
//    bool check = false;

//    for (int j = 0; j < db.Phone.Length; j++)
//    {
//        int count = 0;
//        for (int k = 0; k < db.Phone.Length; k++)
//        {

//            if (db.Phone[j] == db.Phone[k])
//            {
//                count++;
//            }
...
//db13.ForEach(db => Console.WriteLine($"{db.FullName}  {db.Phone}"));
...
//var db14 = debtors.Where(db =>
//{
//    DateTime d = DateTime.Now;
//    while (d.Month != db.BirthDay.Month)
//    {
//        db.Debt -= 500;
//        d = d.AddMonths(1);
//    }
...
//db14.ForEach(db => Console.WriteLine($"{db.FullName}  {db.Debt}"));



//    15)Adindaki ve familyasindaki herflerden "smile" sozunu yaza bileceyimiz borclularin siyahisini cixartmaq

//var db15 = debtors.Where(db => db.FullName.Contains('s') && db.FullName.Contains('m') && db.FullName.Contains('i') && db.FullName.Contains('l') && db.FullName.Contains('e')).ToList();

//db15.ForEach(db => Console.WriteLine(db.FullName));
